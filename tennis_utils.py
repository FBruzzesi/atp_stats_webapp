...
class SackmanScraper:
    ...
    def get_players(self):
        ...
        self.players['birth_date'] = pd.to_datetime(self.players['birth_date'], format='%Y%m%d').dt.date
    ...
    def get_matches(self, years_to_parse=list(range(2000, 2021)), drop_davis=False):
        ...
        t_cols.remove('tourney_id')
        self.matches.drop(t_cols, axis=1, inplace=True)
    ...
    def prepare_matches(self):
        ...
        m = m[np.any(masks.T, axis=1)]
                
        self.matches = (m
                .assign(w_percAce = m['w_ace'].to_numpy()/m['w_svpt'].to_numpy())
                .assign(l_percAce = m['l_ace'].to_numpy()/m['l_svpt'].to_numpy())
                .assign(w_percDf = m['w_df'].to_numpy()/m['w_svpt'].to_numpy())
                .assign(l_percDf = m['l_df'].to_numpy()/m['l_svpt'].to_numpy())
                .assign(w_perc1stIn = m['w_1stIn'].to_numpy() / m['w_svpt'].to_numpy())
                .assign(l_perc1stIn = m['l_1stIn'].to_numpy() / m['l_svpt'].to_numpy())
                .assign(w_perc1stWon = m['w_1stWon'].to_numpy() / m['w_1stIn'].to_numpy())
                .assign(l_perc1stWon = m['l_1stWon'].to_numpy() / m['l_1stIn'].to_numpy())
                .assign(w_perc2ndWon = m['w_2ndWon'].to_numpy() / (m['w_svpt'].to_numpy() - m['w_1stIn'].to_numpy()))
                .assign(l_perc2ndWon = m['l_2ndWon'].to_numpy() / (m['l_svpt'].to_numpy() - m['l_1stIn'].to_numpy()))
                .assign(w_percBpSave = np.where(m['w_bpFaced']==0, 1, m['w_bpSaved'].to_numpy() / m['w_bpFaced'].to_numpy()))
                .assign(l_percBpSave = np.where(m['l_bpFaced']==0, 1, m['l_bpSaved'].to_numpy() / m['l_bpFaced'].to_numpy()))
                .assign(w_SvLost = m['w_bpFaced'].to_numpy() - m['w_bpSaved'].to_numpy())
                .assign(l_SvLost = m['l_bpFaced'].to_numpy() - m['l_bpSaved'].to_numpy())
                ...
                .assign(w_percSvLost = lambda x: x['w_SvLost'].to_numpy()/x['w_SvGms'].to_numpy())
                .assign(l_percSvLost = lambda x: x['l_SvLost'].to_numpy()/x['l_SvGms'].to_numpy())
                .assign(w_returnWon = m['l_svpt'].to_numpy() - (m[['l_1stWon', 'l_2ndWon']].to_numpy().sum(axis=1)))
                .assign(l_returnWon = m['w_svpt'].to_numpy() - (m[['w_1stWon', 'w_2ndWon']].to_numpy().sum(axis=1)))
                .assign(w_percReturnWon = lambda x: x['w_returnWon'].to_numpy()/x['l_svpt'].to_numpy())
                .assign(l_percReturnWon = lambda x: x['l_returnWon'].to_numpy()/x['w_svpt'].to_numpy())
                )
